...
public class Yen {
     ...
     */
    public ArrayList<TpgPath> ksp(TpgNode src, TpgNode dst, int K) {
        // Initialize containers for candidate paths and k shortest paths
        ArrayList<TpgPath> ksp = new ArrayList<>();
        ArrayList<Integer> kspFailures = new ArrayList<>();
        ArrayList<TpgPath> candidates = new ArrayList<>();
        ArrayList<Integer> candidateFailures = new ArrayList<>();
//        BF bf = new BF(g);
        Tpvp tpvp = new Tpvp(g);
        ArrayList<TpgPath> curCandidates = new ArrayList<>();
...
//            K = kthPath.getEdgesize() + 1;
            ...
            for (int k = 1; k < K; k++) {
                ...
                TpgPath previousPath = ksp.get(k-1);
                ...
                /* Iterate over all of the Vertexs in the (k-1)st shortest path except for the target Node; for each Node,
                   ...
                   graph */
                for (int i = 0; i < previousPath.getTpgNodes().size(); i++) {
                    // Initialize a container to store the modified (removed) edges for this Node/iteration
                    ArrayList<TpgEdge> removedEdges = new ArrayList<>();
                    ArrayList<TpgNode> removedNodes = new ArrayList<>();
                    ...
                    TpgNode spurVertex = previousPath.getVertex(previousPath.getTpgNodes().size() - i);
                    // System.out.println("K = "+ k + " i = " + i + " Spur Node " + spurVertex);
                    // Root path = prefix portion of the (k-1)st path up to the spur Node
                    TpgPath rootPath = previousPath.clone(i,previousPath.getTpgNodes().size()-1);
                    ...
                    for(TpgPath p : ksp) {
                        ...
                        TpgPath stub = p.clone(i,previousPath.getTpgNodes().size()-1);
                        ...
                        if (rootPath.equals(stub)) {
                            ...
                            int index = p.getTpgNodes().size() - i - 1;
                            TpgNode repeatNode = p.getTpgNodes().get(index);
//                            TpgEdge re = p.getEdge(index);
//                            TpgEdge removing = re.copy();
                            repeatNode.setValid(false);
                            removedNodes.add(repeatNode);
                            ////System.out.println(re);
                        }
                    }
                    /* Temporarily remove all of the Vertexs in the root path, other than the spur Node, from the graph */
                    removedNodes.addAll(rootPath.getTpgNodes());
                    // Spur path = shortest path from spur Node to target Node in the reduced graph
//                    tpvp.initializeGraph();
//                    tpvp.setSeen(spurVertex, kspCommMap.get(k-1).get(spurVertex));

                    TpgPath spurPath = tpvp.shortest(src, spurVertex);
                    ...
                    if (spurPath != null) {
                        ...
                        TpgPath totalPath = spurPath.clone();
                        //System.out.println();
